...
\begin{itemize}
    \item \textbf{Custom Particle Flow Rendering:} The canvas simulation animates flow lines to represent data links. Table~\ref{tab:force_graph_params} summarizes the parameters and variables used in the animation rendering loop, with the complete source code detailed in Appendix N.
    ...
    \begin{tabular}{lll}
    \toprule
    \textbf{Rendering Parameter} & \textbf{Target Configuration} & \textbf{Role in Animation / Physics} \\
    \midrule
    Stroke Style & \texttt{rgba(59, 130, 246, 0.15)} & Draws faint static connection lines between nodes \\
    Time Delta & \texttt{Date.now() * 0.003} & Time scalar coefficient driving uniform flow velocity \\
    Link Progress & \texttt{(time + index * 0.1) \% 1.0} & Normalizes particle position relative to line segment length \\
    Coordinates & \texttt{px, py = source + delta * progress} & Linearly interpolates particle location on Canvas \\
    Particle Size & \texttt{Radius = 2.5} & Diameter size of the glowing particle node \\
    Fill Style & \texttt{\#3B82F6} & Neon blue particle fill color matching CTI styling \\
    \bottomrule
    \end{tabular}
    \caption{D3 Force Graph Link Rendering Parameters.}
    \label{tab:force_graph_params}
    \end{table}
