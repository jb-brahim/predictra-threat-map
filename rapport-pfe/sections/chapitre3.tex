...
%     % Boundary
%     \draw[draw=charcoal, fill=charcoal!2, thick, rounded corners] (0,-2.5) rectangle (8.5,2.5);
%     \node at (4.25,2.1) {\textbf{Sprint 1: Harvester Boundary}};
%     
%     % Actors
%     \node[draw=charcoal, top color=white, bottom color=gray!15, thick, drop shadow, rounded corners, minimum width=2cm, minimum height=1cm, align=center] (feed) at (-2.5, 0) {\textbf{External API}\\\textbf{Feed}};
%     \node[draw=charcoal, top color=white, bottom color=gray!15, thick, drop shadow, rounded corners, minimum width=2cm, minimum height=1cm, align=center] (server) at (11, 0) {\textbf{Express}\\\textbf{Server}};
% 
%     % Use Cases
%     \node[draw, ellipse, top color=white, bottom color=cyberblue!15, drop shadow, minimum width=3.5cm, minimum height=0.9cm] (uc1) at (4.25, 1) {Fetch Threat Feed};
%     \node[draw, ellipse, top color=white, bottom color=cyberblue!15, drop shadow, minimum width=3.5cm, minimum height=0.9cm] (uc2) at (4.25, -1) {Enrich \& Store Payload};
% 
%     % Links
%     \draw[thick, -] (feed) -- (uc1);
%     \draw[thick, -] (server) -- (uc1);
%     \draw[thick, -] (server) -- (uc2);
% \end{tikzpicture}
\includegraphics[width=0.95\textwidth]{assets/s1_usecase.pdf}
...
%     % Express Server
%     \node[draw, rectangle, top color=white, bottom color=cyberblue!5, drop shadow, rounded corners=2pt, text width=6.5cm, align=left] (express) at (0,0) {
%         \textbf{Express Server (/api/feed)} \\
%         \hrulefill \\
%         + clients : Response[] \\
%         + pendingBatch : ThreatEvent[] \\
%         \hrulefill \\
%         + addClient(req, res) : void \\
%         + broadcast(events) : void \\
%         - flushBatch() : void (every 300ms)
%     };
%     
%     % Zustand Store
%     \node[draw, rectangle, top color=white, bottom color=cyberblue!5, drop shadow, rounded corners=2pt, text width=6.5cm, align=left] (zustand) at (8,0) {
%         \textbf{Zustand Store (streamStore)} \\
%         \hrulefill \\
%         + activeArcs : ArcData[] \\
%         + eventsBuffer : RingBuffer \\
%         \hrulefill \\
%         + initStream() : void \\
%         + addEvents(payload) : void \\
%         + tick(now) : void (60fps)
%     };
% 
%     \draw[thick, latex-] (express) -- node[above, scale=0.8] {sync} (zustand);
% \end{tikzpicture}
\includegraphics[width=0.95\textwidth]{assets/s1_class.pdf}
...
% \begin{tikzpicture}[scale=0.85, every node/.style={transform shape}]
%     % Lifelines
%     \draw[dashed, thick] (0,0) -- (0,-6) node[below] {Scraper Service};
%     \draw[dashed, thick] (3.8,0) -- (3.8,-6) node[below] {Enrichment.js};
%     \draw[dashed, thick] (7.5,0) -- (7.5,-6) node[below] {Server.js Queue};
%     \draw[dashed, thick] (11.2,0) -- (11.2,-6) node[below] {MongoDB Atlas};
% 
%     % Actors
%     \coordinate (feed) at (0,0);
%     \node[draw=charcoal, top color=white, bottom color=gray!15, thick, drop shadow, rounded corners, align=center, minimum width=2cm, minimum height=0.6cm] at (0,0.3) {External API};
% 
%     \coordinate (scraper) at (3.8,0);
%     \node[draw=charcoal, top color=white, bottom color=cyberblue!15, thick, drop shadow, rounded corners, align=center, minimum width=2cm, minimum height=0.6cm] at (3.8,0.3) {Scraper.js};
% 
%     \coordinate (enrich) at (7.5,0);
%     \node[draw=charcoal, top color=white, bottom color=cyberblue!15, thick, drop shadow, rounded corners, align=center, minimum width=2cm, minimum height=0.6cm] at (7.5,0.3) {Enrichment};
% 
%     \coordinate (db) at (11.2,0);
%     \node[draw=charcoal, top color=white, bottom color=cyberblue!15, thick, drop shadow, rounded corners, align=center, minimum width=2cm, minimum height=0.6cm] at (11.2,0.3) {MongoDB};
% 
%     % Sequence Messages
%     \draw[-latex, thick] (0,-1) -- node[above, scale=0.8] {Poll / Stream Raw Event} (3.8,-1);
%     
%     \draw[-latex, thick] (3.8,-1.8) -- node[above, xshift=1.2cm, scale=0.8] {Geolocate IP (geoip-lite)} (3.8,-2.3);
%     \draw[dashed] (3.8,-2.3) -- (3.8,-2.5);
%     
%     \draw[-latex, thick] (3.8,-2.8) -- node[above, xshift=1.3cm, scale=0.8] {Enrich Sector (5-Layer Heuristics)} (3.8,-3.3);
%     \draw[dashed] (3.8,-3.3) -- (3.8,-3.5);
% 
%     \draw[-latex, thick] (3.8,-3.8) -- node[above, scale=0.8] {broadcast("attack", enrichedData)} (7.5,-3.8);
%     \draw[-latex, thick] (7.5,-4.6) -- node[above, scale=0.8] {insertMany() (bulk $\ge 25$ events)} (11.2,-4.6);
%     \draw[dashed, -latex] (11.2,-5.3) -- node[above, scale=0.8] {Write Acknowledged} (7.5,-5.3);
% \end{tikzpicture}
\includegraphics[width=0.95\textwidth]{assets/s1_sequence.pdf}
...
\begin{itemize}
    \item \textbf{Source Endpoint \& Protocol:} Establishes a direct, long-lived, secure Server-Sent Events (SSE) HTTP stream connection to Checkpoint's corporate live threat telemetry server:
    \begin{verbatim}
    Protocol: HTTP/1.1 SSE (Streaming GET)
    Endpoint: https://threatmap.checkpoint.com/feed/live
    Security: TLS 1.3 Transport Encryption
    \end{verbatim}
    \item \textbf{Raw Response Format:} The scraper receives newline-separated SSE packets, containing real-time corporate firewall records:
    \begin{verbatim}
    event: threat
    data: {
      "srcIp": "194.26.135.8",
...
\begin{itemize}
    \item \textbf{Source Endpoint \& Protocol:} Establishes a real-time Web Socket connection using the \texttt{socket.io-client} protocol to Bitdefender's global threat intelligence sharing hub:
    \begin{verbatim}
    Protocol: WebSocket (ws-transport)
    Endpoint: wss://hub.bitdefender.com/threat-telemetry
    Channels: 13 Active Threat Vector Channels
    \end{verbatim}
    \item \textbf{Raw Response Format:} The WebSocket channel broadcasts binary encoded payloads, which are converted by the socket client into structured JSON frames:
    \begin{verbatim}
    {
      "event": "detection",
      "channel": "ransomware",
...
\begin{itemize}
    \item \textbf{Source Endpoint \& Protocol:} Polls Fortinet's active exploit telemetry API using standard stateless HTTP queries:
    \begin{verbatim}
    Protocol: HTTP/1.1 REST (GET)
    Endpoint: https://fortiguard.com/api/threats/active
    Headers: User-Agent: Predictra-Ingestion-Agent/1.0
    \end{verbatim}
    \item \textbf{Raw Response Format:} Fetches structured JSON arrays containing active exploit campaigns:
    \begin{verbatim}
    [
      {
        "srcCountry": "Germany",
...
\begin{itemize}
    \item \textbf{Source Endpoint \& Protocol:} Queries abuse.ch's URLhaus API to retrieve newly identified malicious URL distribution nodes:
    \begin{verbatim}
    Protocol: HTTP/1.1 REST (POST)
    Endpoint: https://urlhaus-api.abuse.ch/v1/urls/recent/
    Frequency: Polls every 90 seconds
    \end{verbatim}
    \item \textbf{Raw Response Format:} Returns a JSON dictionary containing a list of online, active malware hosting endpoints:
    \begin{verbatim}
    {
      "query_status": "ok",
      "urls": [
...
\begin{itemize}
    \item \textbf{Source Endpoint \& Protocol:} Polls abuse.ch's Feodo Tracker database of active Botnet command-and-control (C2) servers:
    \begin{verbatim}
    Protocol: HTTP/1.1 REST (GET)
    Endpoint: https://feodotracker.abuse.ch/
              downloads/ipblocklist.csv
    Frequency: Polls every 60 seconds
    \end{verbatim}
    \item \textbf{Raw Response Format:} The Feodo feed returns raw CSV data blocks. Lines prefixed with \texttt{\#} are comment lines. Active lines represent active botnet nodes:
    \begin{verbatim}
...
\begin{itemize}
    \item \textbf{Source Endpoint \& Protocol:} Queries the AlienVault Open Threat Exchange REST API, utilizing custom developer API authentication tokens:
    \begin{verbatim}
    Protocol: HTTP/1.1 REST (GET)
    Endpoint: https://otx.alienvault.com/api/v1/pulses/subscribed
    Headers: X-OTX-API-KEY: <custom_otx_api_token>
    \end{verbatim}
    \item \textbf{Raw Response Format:} Returns complex, nested threat "pulses" containing clusters of related Indicators of Compromise (IOCs):
    \begin{verbatim}
    {
      "count": 1,
      "results": [
    ...
    \end{verbatim}
    \item \textbf{Operation Mechanism:} Polls every 5 minutes, pulling recent indicators of compromise (IOCs) from subscribed threat pulses. It parses IP addresses, hashes, and vulnerability details.
...
\begin{itemize}
    \item \textbf{Source Endpoint \& Protocol:} Polls the Ransomlook.io API endpoint monitoring active ransomware extortion leak sites:
    \begin{verbatim}
    Protocol: HTTP/1.1 REST (GET)
    Endpoint: https://ransomlook.io/api/posts/recent
    Frequency: Polls every 10 minutes
    \end{verbatim}
    \item \textbf{Raw Response Format:} Fetches ransomware extortion listings:
    \begin{verbatim}
    [
      {
        "group": "LockBit 3.0",
...
\begin{itemize}
    \item \textbf{Source Endpoint \& Protocol:} Polls the MontySecurity command-and-control tracker endpoint to monitor active listener framework IPs:
    \begin{verbatim}
    Protocol: HTTP/1.1 REST (GET)
    Endpoint: https://github.com/c2/active/c2.json
    Frequency: Polls every 15 minutes
    \end{verbatim}
    \item \textbf{Raw Response Format:} Fetches JSON arrays listing active command framework installations:
    \begin{verbatim}
    [
      {
        "ip": "185.220.101.9",
...
\begin{itemize}
    \item \textbf{Source Endpoint \& Protocol:} Accesses local JSON data structures cached inside the backend configuration folder:
    \begin{verbatim}
    Protocol: In-Memory JSON Parsing (File Read)
    Cache Directory: backend/config/misp-galaxy/
    Frequency: Reloads cache every 6 hours
    \end{verbatim}
    \item \textbf{Raw Response Format:} Loads detailed threat clusters, mapping groups like APT29 or APT28.
...
%     % Boundary
%     \draw[draw=charcoal, fill=charcoal!2, thick, rounded corners] (0,-2) rectangle (8.5,2);
%     \node at (4.25,1.6) {\textbf{Sprint 2: 3D Visualizer Boundary}};
% 
%     % Actors
...
% 
%     % Use Cases
%     \node[draw, ellipse, top color=white, bottom color=cyberblue!15, drop shadow, minimum width=3.8cm, minimum height=0.9cm] (uc1) at (4.25, 0.7) {Monitor 3D Spatial Globe};
%     \node[draw, ellipse, top color=white, bottom color=cyberblue!15, drop shadow, minimum width=3.8cm, minimum height=0.9cm] (uc2) at (4.25, -0.7) {Toggle 2D Projection};
% 
%     % Links
%     \draw[thick, -] (analyst) -- (uc1);
%     \draw[thick, -] (analyst) -- (uc2);
% \end{tikzpicture}
\includegraphics[width=0.95\textwidth]{assets/s2_usecase.pdf}
...
% \begin{tikzpicture}[scale=0.85, every node/.style={transform shape}]
%     % Lifelines
%     \draw[dashed, thick] (0,0) -- (0,-5.5) node[below] {Express (/api/feed)};
%     \draw[dashed, thick] (3.8,0) -- (3.8,-5.5) node[below] {EventSource Client};
%     \draw[dashed, thick] (7.5,0) -- (7.5,-5.5) node[below] {Zustand Store};
%     \draw[dashed, thick] (11.2,0) -- (11.2,-5.5) node[below] {Three.js Canvas};
% 
%     % Headers
%     \node[draw, fill=cyberblue!10, minimum width=2cm, minimum height=0.6cm] at (0,0.3) {Express Server};
%     \node[draw, fill=cyberblue!10, minimum width=2cm, minimum height=0.6cm] at (3.8,0.3) {EventSource};
%     \node[draw, fill=cyberblue!10, minimum width=2cm, minimum height=0.6cm] at (7.5,0.3) {Zustand Store};
%     \node[draw, fill=cyberblue!10, minimum width=2cm, minimum height=0.6cm] at (11.2,0.3) {WebGL Globe};
% 
%     % Messages
%     \draw[-latex, thick] (0,-1) -- node[above, scale=0.8] {SSE broadcast (300ms flush)} (3.8,-1);
%     \draw[-latex, thick] (3.8,-2) -- node[above, scale=0.8] {onmessage: addEvents()} (7.5,-2);
%     
%     \draw[-latex, thick] (7.5,-2.8) -- node[above, xshift=1.2cm, scale=0.8] {latLonToVector3()} (7.5,-3.3);
%     \draw[dashed] (7.5,-3.3) -- (7.5,-3.5);
%     
%     \draw[-latex, thick] (7.5,-4) -- node[above, scale=0.8] {Append new ArcData \& MarkerData} (11.2,-4);
%     \draw[-latex, thick] (11.2,-4.8) -- node[above, scale=0.8] {useFrame: Render dynamic bezier trails} (11.2,-5.3);
% \end{tikzpicture}
\includegraphics[width=0.95\textwidth]{assets/s2_sequence.pdf}
...
\begin{itemize}
    ...
    To illustrate this shader logic, our active GLSL implementation is detailed below:
    
    \textbf{GLSL Vertex Shader (\texttt{atmosphereVertex.glsl}):}
    \begin{verbatim}
    ...
    \textbf{GLSL Fragment Shader (\texttt{atmosphereFragment.glsl}):}
    \begin{verbatim}
    varying vec3 vNormal;
    void main() {
      // Calculate atmospheric scattering glow strength
      float dotProd = dot(vNormal, vec3(0, 0, 1.0));
      float intensity = pow(0.6 - dotProd, 2.0);
      // Neon Cyberblue color
      vec3 atmosphericColor = vec3(0.23, 0.51, 0.96);
      gl_FragColor = vec4(atmosphericColor, 0.4) * intensity;
    }
    \end{verbatim}
