...
\begin{tikzpicture}[node distance=2cm, scale=0.85, every node/.style={transform shape}]
    ...
    % Actors
    \node[draw=charcoal, fill=cyberblue!10, thick, minimum width=2cm, minimum height=1cm, align=center] (feed) at (-2.5, 0) {\textbf{External API}\\\textbf{Feed}};
    \node[draw=charcoal, fill=cyberblue!10, thick, minimum width=2cm, minimum height=1cm, align=center] (server) at (11, 0) {\textbf{Express}\\\textbf{Server}};

    % Use Cases
    \node[draw, ellipse, fill=white, minimum width=3.5cm, minimum height=0.9cm] (uc1) at (4.25, 1) {Fetch Threat Feed};
    \node[draw, ellipse, fill=white, minimum width=3.5cm, minimum height=0.9cm] (uc2) at (4.25, -1) {Enrich \& Store Payload};
...
\begin{tikzpicture}[scale=0.85, every node/.style={transform shape}]
    % ThreatEvent Class
    \node[draw, rectangle, fill=cyberblue!5, text width=6.5cm, align=left] (threatModel) at (0,0) {
        \textbf{ThreatEvent (Model)} \\
        \hrulefill \\
        + a\_c : Number (count) \\
        + a\_n : String (name) \\
        + a\_t : String (type: exploit or malware) \\
        + s\_co / d\_co : String (ISO country) \\
        + s\_la / s\_lo / d\_la / d\_lo : Number \\
        + s\_ip / d\_ip : String \\
        + source\_api : String \\
        + timestamp : Date (Expires in 30 days) \\
        \hrulefill \\
        + insertMany() : Promise
    };
    
    % EnrichmentService Class
    \node[draw, rectangle, fill=cyberblue!5, text width=6.5cm, align=left] (enrichment) at (8,0) {
        ...
        - rdapCache : Map \\
        \hrulefill \\
        + getEnrichedSector(ip, details) : String \\
        + getIpOrganization(ip) : String \\
        + calculateSeverity(details) : String
    };

    \draw[thick, latex-] (threatModel) -- node[above, scale=0.8] {calls} (enrichment);
\end{tikzpicture}
...
\begin{tikzpicture}[scale=0.85, every node/.style={transform shape}]
    ...
    \draw[dashed, thick] (3.5,0) -- (3.5,-6) node[below] {Enrichment.js};
    \draw[dashed, thick] (7,0) -- (7,-6) node[below] {Server.js Queue};
    \draw[dashed, thick] (10.5,0) -- (10.5,-6) node[below] {MongoDB Atlas};

    % Headers
    \node[draw, fill=cyberblue!10, minimum width=2cm, minimum height=0.6cm] at (0,0.3) {Scrapers};
    \node[draw, fill=cyberblue!10, minimum width=2cm, minimum height=0.6cm] at (3.5,0.3) {Enrichment};
    \node[draw, fill=cyberblue!10, minimum width=2cm, minimum height=0.6cm] at (7,0.3) {Server};
    \node[draw, fill=cyberblue!10, minimum width=2cm, minimum height=0.6cm] at (10.5,0.3) {MongoDB};

    % Sequence Messages
    \draw[-latex, thick] (0,-1) -- node[above, scale=0.8] {Poll / Stream Raw Event} (3.5,-1);
    
    \draw[-latex, thick] (3.5,-1.8) -- node[above, xshift=1.2cm, scale=0.8] {Geolocate IP (geoip-lite)} (3.5,-2.3);
    \draw[dashed] (3.5,-2.3) -- (3.5,-2.5);
    
    \draw[-latex, thick] (3.5,-2.8) -- node[above, xshift=1.3cm, scale=0.8] {Enrich Sector (5-Layer Heuristics)} (3.5,-3.3);
    \draw[dashed] (3.5,-3.3) -- (3.5,-3.5);

    \draw[-latex, thick] (3.5,-3.8) -- node[above, scale=0.8] {broadcast("attack", enrichedData)} (7,-3.8);
    \draw[-latex, thick] (7,-4.6) -- node[above, scale=0.8] {insertMany() (bulk $\ge 25$ events)} (10.5,-4.6);
    \draw[dashed, -latex] (10.5,-5.3) -- node[above, scale=0.8] {Write Acknowledged} (7,-5.3);
\end{tikzpicture}
...
\begin{tikzpicture}[node distance=2.2cm, scale=0.85, every node/.style={transform shape}]
    ...
    % Use Cases
    \node[draw, ellipse, fill=white, minimum width=3.8cm, minimum height=0.9cm] (uc1) at (4.25, 0.7) {Monitor 3D Spatial Globe};
    \node[draw, ellipse, fill=white, minimum width=3.8cm, minimum height=0.9cm] (uc2) at (4.25, -0.7) {Toggle 2D Projection};
