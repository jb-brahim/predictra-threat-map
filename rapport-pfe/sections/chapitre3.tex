...
\begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{3.8cm}XX}
\hline
\rowcolor{charcoal}
...
\arrayrulecolor{charcoal!30}
\rowcolors{2}{cyberblue!5}{white}
\begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{3.2cm}>{\raggedright\arraybackslash}p{2.0cm}>{\raggedright\arraybackslash}p{1.4cm}X}
\hline
\rowcolor{charcoal}
\thc{BSON Field} & \thc{Logical Name} & \thc{Type} & \thc{Description / Constraints} \\
\hline
...
\begin{itemize}
    ...
    \hline
    \rowcolor{charcoal}
    \thc{Specification Parameter} & \thc{Configuration Value} \\
    \hline
    ...
    \item \textbf{Expected Payload Schema:} The stream broadcasts SSE threat packets separated by newlines. The parsed JSON data attributes are defined in Table~\ref{tab:payload_checkpoint} (with the raw payload example detailed in Appendix I.1).
    \begin{table}[H]
    \centering
    \small
    \arrayrulecolor{charcoal!30}
    ...
    \hline
    \rowcolor{charcoal}
    \thc{JSON Attribute} & \thc{Data Type} & \thc{Description / Constraints} \\
    \hline
...
\begin{itemize}
    ...
    \hline
    \rowcolor{charcoal}
    \thc{CSV Attribute} & \thc{Data Type} & \thc{Description / Constraints} \\
    \hline
...
\arrayrulecolor{charcoal!30}
\rowcolors{2}{cyberblue!5}{white}
\begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{2.8cm}>{\raggedright\arraybackslash}p{5.2cm}X}
\hline
\rowcolor{charcoal}
\thc{Use Case} & \thc{Actor/Trigger} & \thc{Main Workflow} \\
\hline
...
To bridge the gap between geographical databases (which express threat locations using standard Latitude/Longitude) and the 3D visual workspace (which requires Cartesian coordinates), we designed custom geocoding and curve-generation pipelines.

\subsubsection{Spatial Projection Mathematics (Latitude/Longitude to Vector3)}
To display coordinates on the 3D globe, the system converts geographic coordinates (latitude and longitude) into 3D Cartesian coordinates (X, Y, Z). This conversion maps the coordinates to a sphere of radius 1.0. The formulas scale the angles from degrees to radians and map them to the corresponding 3D positions. The system also aligns the coordinates with the screen axes to ensure geographical boundaries are displayed in the correct hemispheres.

\subsubsection{Trigonometric Derivation of Spherical Linear Interpolation (SLERP)}
To render an attack line moving from a starting country to a destination country across the globe's surface, a simple straight line is incorrect because it would go through the interior of the sphere. Instead, the system uses Spherical Linear Interpolation (SLERP) to calculate points along the curve of the sphere. This ensures that the attack trail follows the curvature of the Earth at a constant speed.

\subsubsection{Parametric Quadratic Bezier Fly-Arcs}
To make the attack lines visually distinct and avoid clutter, they are designed as elevated arcs that rise above the sphere's surface, reaching their highest point at the midpoint of their flight:
\begin{enumerate}
    \item The system first computes a surface midpoint coordinates vector using spherical interpolation between the source and destination.
    \item An elevated control point is then projected radially outward from the center of the sphere. The height is dynamically scaled: longer distances arch higher, while shorter paths remain closer to the surface, organizing the visualization.
    \item These points are used to generate a smooth Bezier curve, providing a clean neon volumetric path.
\end{enumerate}
...
Our primary globe mesh in \texttt{globe/Earth.tsx} implements a performant custom material for rendering:
\begin{itemize}
    \item \textbf{Atmospheric Glow:} To create a premium visual experience, the globe uses a custom shader that calculates a glowing border (Fresnel effect) around the silhouette of the globe, which fades smoothly towards the center.
    ...
    \hline
    \rowcolor{charcoal}
    \thc{Variable Name} & \thc{GLSL Type} & \thc{Role / Computational Purpose} \\
    \hline
