...
function resolveCountryCode(actor) { // Define helper function to extract country codes from actor entries
  ...
  if (actor.meta?.country) return actor.meta.country.toUpperCase(); // Return capitalized country metadata attribute if present
  // State sponsor field (full name)
  const sponsor = actor.meta?.['cfr-suspected-state-sponsor']; // Read suspected state sponsor string from metadata
  if (sponsor) { // If sponsor property exists
    const cc = COUNTRY_NAME_TO_CC[sponsor.toLowerCase()]; // Perform dictionary lookup matching lowercase name
    if (cc) return cc; // Return mapped two-letter ISO code if found
  } // End of sponsor conditional block
  return null; // Return null if country metadata cannot be resolved
...
function resolveVictimCountries(actor) { // Define helper function to extract targeted victim countries from actor metadata
  const victims = actor.meta?.['cfr-suspected-victims'] || []; // Extract victims names list or default to empty list
  return victims // Process victim entries
    .map(v => COUNTRY_NAME_TO_CC[v.toLowerCase()]) // Map name strings to two-letter country codes via dictionary lookup
    .filter(Boolean); // Filter out null/undefined lookup results
...
function resolveAttackType(actor) { // Define helper to select threat category based on incident history descriptions
  const incident = (actor.meta?.['cfr-type-of-incident'] || '').toLowerCase(); // Read incident type string, default to empty, convert to lowercase
  if (incident.includes('espionage')) return 'exploit'; // Map espionage keywords to exploit category
  if (incident.includes('sabotage') || incident.includes('destruct')) return 'malware'; // Map sabotage/destruction keywords to malware category
